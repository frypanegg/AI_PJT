# -*- coding: utf-8 -*-
"""규칙 기반 오프라인 조직운영 대화형 도우미 (Action Guide Agent 데모).

외부 API를 호출하지 않고 로컬에서 완전히 동작한다. 사용자의 자유 입력을 14개
중분류 키워드와 매칭해 해당 영역의 해석 카드(catalog.MID_INTERPRETATION)와
조직의 2026 실제 응답 비중을 조합한 guardrail-safe 응답을 만든다. 매칭되지
않으면 단정하지 않고 선택 가능한 주제 목록을 안내한다.
"""
...
DISCLAIMER = (
    "이 대화는 사전 정의된 해석 로직 기반 데모이며, 평가·처방이 아닌 점검 방향을 "
    "제안하는 참고용입니다. 특정 원인을 단정하지 않으며 외부 서버로 데이터를 전송하지 않습니다."
)
...
def category_reply(mid: str, mid_df) -> str:
    ...
    dominant = max(("긍정", "중립", "부정"), key=lambda k: row[k])
    lines = [
