...
def three_year_trend(org: str, mid: str | None = None) -> list[dict]:
...
def executive_summary(org: str, overall: dict, top_pos: list, top_neg: list) -> str:
...
def trend_delta(org: str, mid: str) -> float:
    trend = three_year_trend(org, mid)
...
def report_payload(org_key: str) -> dict:
    ...
    if is_suppressed("2026", org):
        return {"suppressed": True, "org": org, "company": org_row["company"]}

    overall = overall_ratio("2026", org)
    mids = mid_ratios("2026", org)
    questions = question_ratios("2026", org)
    top_pos = top_mids("2026", org, "긍정")
    top_neg = top_mids("2026", org, "부정")
    respondents = respondent_count("2026", org)
    ...
    return {
        ...
        "top_neg": top_neg,
        "open_texts": {
            qid: {"label": label, "items": top_open_texts("2026", org, qid)}
            for qid, label in OPEN_TEXT_QUESTIONS.items()
        },
        "executive_summary": executive_summary(org, overall, top_pos, top_neg),
        "overall_trend": three_year_trend(org),
        "mid_trends": {m["중분류"]: three_year_trend(org, m["중분류"]) for m in mids},
        "improved": improved,
...
def admin_payload() -> dict:
    company_wide = company_wide_ratio("2026")
    rows = []
    for o in list_orgs():
        org = o["org"]
        if is_suppressed("2026", org):
            continue
        ratio = overall_ratio("2026", org)
